...
const Healthcare = () => {
  ...
  const [error, setError] = useState(null); // Add this line to initialize error state
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8080/api/appointment/appointments/user/${userId}`);
